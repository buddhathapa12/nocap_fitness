...
// Firebase imports removed as login system is being removed
...
const App = () => {
    ...
    const handleGeneratePlan = async () => {
        ...
        const dietarySchema = {
            ...
            required: ["why_this_plan_works", "meal_plan", "important_considerations"]
        };
        ...
        const holisticWellnessSchema = {
            ...
            required: ["mental_wellness_modules", "sleep_optimization_tips", "recovery_protocols", "psychological_prompts"]
        };
        ...
        const gamificationSchema = {
            ...
            required: ["user_level_info", "current_xp", "tokens_earned", "dynamic_challenges", "mastery_paths"]
        };
        ...
        const communitySchema = {
            ...
            required: ["squad_suggestion", "content_sharing_ideas", "expert_integration_info", "accountability_buddy_idea"]
        };
        ...
        const psychologicalInsightsSchema = {
            ...
            required: ["mood_trend_analysis", "stress_correlation_insight", "consistency_motivation", "psychological_nudges"]
        };
        ...
        try {
            ...
            const generatedDietaryPlan = await callGeminiApi(dietaryPrompt, true, dietarySchema);
            ...
            const generatedFitnessPlan = await callGeminiApi(fitnessPrompt, true, fitnessSchema);
            ...
            const generatedHolisticWellness = await callGeminiApi(holisticWellnessPrompt, true, holisticWellnessSchema);
            ...
            const generatedGamification = await callGeminiApi(gamificationPrompt, true, gamificationSchema);
            ...
            const generatedCommunity = await callGeminiApi(communityPrompt, true, communitySchema);
            ...
            const generatedPsychologicalInsights = await callGeminiApi(psychologicalInsightsPrompt, true, psychologicalInsightsSchema);
    ...
    const handleGetAnswer = async () => {
        ...
        Holistic Wellness: Mental Wellness: ${holisticWellness.mental_wellness_modules || ''}, Sleep: ${holisticWellness.sleep_optimization_tips || ''}, Recovery: ${holisticWellness.recovery_protocols || ''}, Psychological Prompts: ${holisticWellness.psychological_prompts || ''}
        Gamification: Level: ${gamification.user_level_info || ''}, Challenges: ${gamification.dynamic_challenges?.join(', ') || ''}, Mastery Paths: ${gamification.mastery_paths?.join(', ') || ''}
        Community: Squads: ${community.squad_suggestion || ''}, Sharing: ${community.content_sharing_ideas || ''}, Expert Integration: ${community.expert_integration_info || ''}, Accountability Buddy: ${community.accountability_buddy_idea || ''}
        Psychological Insights: Mood: ${psychologicalInsights.mood_trend_analysis || ''}, Stress: ${psychologicalInsights.stress_correlation_insight || ''}, Nudges: ${psychologicalInsights.psychological_nudges?.join(', ') || ''}
        User Question: ${questionInput}`;

        try {
            const answer = await callGeminiApi(fullContext);
            if (answer) {
                setQaPairs(prev => [...prev, { question: questionInput, answer }]);
    ...
    const DisplayDietaryPlan = ({ planContent }) => (
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
            <h2 className="text-2xl font-bold text-blue-800 mb-4">📋 Your Personalized Dietary Plan</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🎯 Why this plan works</h3>
                    ...
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🍽️ Meal Plan</h3>
                    <div className="bg-gray-50 p-4 rounded-lg text-gray-800">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.meal_plan || "Plan not available"}</pre>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">⚠️ Important Considerations</h3>
                    <div className="space-y-3">
                        {(planContent.important_considerations || "").split('\n').map((consideration, index) => (
                            consideration.trim() && (
                                <div key={index} className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-3 rounded-lg">
                                    {consideration.trim()}
                                </div>
                            )
                        ))}
    ...
    const DisplayFitnessPlan = ({ planContent }) => (
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
            <h2 className="text-2xl font-bold text-green-800 mb-4">💪 Your Personalized Fitness Plan</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🏋️‍♂️ Exercise Routine</h3>
                    <div className="bg-gray-50 p-4 rounded-lg text-gray-800">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.routine || "Routine not available"}</pre>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">💡 Pro Tips</h3>
                    <div className="space-y-3">
                        {(planContent.tips || "").split('\n').map((tip, index) => (
                            tip.trim() && (
                                <div key={index} className="bg-blue-50 border border-blue-200 text-blue-800 p-3 rounded-lg">
                                    {tip.trim()}
                                </div>
                            )
                        ))}
    ...
    const DisplayHolisticWellness = ({ planContent }) => (
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
            <h2 className="text-2xl font-bold text-purple-800 mb-4">🧘‍♀️ Holistic Wellness & Recovery</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🧠 Mental Wellness Modules</h3>
                    <div className="bg-purple-50 border border-purple-200 text-purple-800 p-4 rounded-lg mb-4">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.mental_wellness_modules || "Suggestions not available"}</pre>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">😴 Sleep Optimization Tips</h3>
                    <div className="bg-purple-50 border border-purple-200 text-purple-800 p-4 rounded-lg mb-4">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.sleep_optimization_tips || "Tips not available"}</pre>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🩹 Recovery Protocols</h3>
                    <div className="bg-purple-50 border border-purple-200 text-purple-800 p-4 rounded-lg">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.recovery_protocols || "Protocols not available"}</pre>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">💡 Psychological Prompts</h3>
                    <div className="bg-purple-50 border border-purple-200 text-purple-800 p-4 rounded-lg">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.psychological_prompts || "Prompts not available"}</pre>
                    </div>
                </div>
            </div>
        </div>
    ...
    const DisplayGamification = ({ planContent }) => (
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
            <h2 className="text-2xl font-bold text-orange-800 mb-4">🎮 Gamification & Rewards</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">✨ Your Progress</h3>
                    <div className="bg-orange-50 border border-orange-200 text-orange-800 p-4 rounded-lg mb-4">
                        <p className="mb-1"><strong>Level:</strong> {planContent.user_level_info || 'N/A'}</p>
                        <p className="mb-1"><strong>XP:</strong> {planContent.current_xp || 0}</p>
                        <p><strong>Tokens:</strong> {planContent.tokens_earned || 0}</p>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🚀 Dynamic Challenges</h3>
                    <div className="bg-orange-50 border border-orange-200 text-orange-800 p-4 rounded-lg mb-4">
                        <ul className="list-disc list-inside">
                            {(planContent.dynamic_challenges || []).map((challenge, index) => (
                                <li key={index}>{challenge}</li>
                            ))}
                            {(planContent.dynamic_challenges?.length === 0 || !planContent.dynamic_challenges) && <li>No challenges suggested.</li>}
                        </ul>
                    </div>
                </div>
                <div className="md:col-span-2">
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🏆 Mastery Paths</h3>
                    <div className="bg-orange-50 border border-orange-200 text-orange-800 p-4 rounded-lg">
                        <ul className="list-disc list-inside">
                            ...
                            {(planContent.mastery_paths?.length === 0 || !planContent.mastery_paths) && <li>No mastery paths suggested.</li>}
                        </ul>
                    </div>
                </div>
            </div>
    ...
    const DisplayCommunity = ({ planContent }) => (
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
            <h2 className="text-2xl font-bold text-teal-800 mb-4">🤝 Community & Social Connectivity</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">👥 Squad Suggestion</h3>
                ...
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">📸 Content Sharing Ideas</h3>
                    <div className="bg-teal-50 border border-teal-200 text-teal-800 p-4 rounded-lg mb-4">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.content_sharing_ideas || "No ideas available"}</pre>
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">👩‍🏫 Expert Integration</h3>
                    <div className="bg-teal-50 border border-teal-200 text-teal-800 p-4 rounded-lg">
                        <pre className="whitespace-pre-wrap font-sans text-sm">{planContent.expert_integration_info || "No information available"}</pre>
                    </div>
                </div>
                <div className="md:col-span-3">
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🤝 Accountability Buddy</h3>
                    <div className="bg-teal-50 border border-teal-200 text-teal-800 p-4 rounded-lg">
    ...
    const DisplayPsychologicalInsights = ({ insightsContent }) => (
        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
            <h2 className="text-2xl font-bold text-pink-800 mb-4">🧠 Psychological Insights & Nudges</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">📊 Mood Trend Analysis</h3>
                ...
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">🔗 Stress Correlation Insight</h3>
                    <div className="bg-pink-50 border border-pink-200 text-pink-800 p-4 rounded-lg mb-4">
                        {insightsContent.stress_correlation_insight || "No insight available"}
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">📈 Consistency Motivation</h3>
                    <div className="bg-pink-50 border border-pink-200 text-pink-800 p-4 rounded-lg">
                        {insightsContent.consistency_motivation || "No motivation tip available"}
                    </div>
                </div>
                <div>
                    <h3 className="text-xl font-semibold text-gray-700 mb-2">✨ Psychological Nudges</h3>
                    <div className="bg-pink-50 border border-pink-200 text-pink-800 p-4 rounded-lg">
                        <ul className="list-disc list-inside">
                            {(insightsContent.psychological_nudges || []).map((nudge, index) => (
                                <li key={index}>{nudge}</li>
                            ))}
                            {(insightsContent.psychological_nudges?.length === 0 || !insightsContent.psychological_nudges) && <li>No nudges available.</li>}
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 font-inter text-gray-900 p-4 sm:p-8">
            ...
            <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl p-6 sm:p-10">
                ...
                <div className="bg-indigo-700 text-white p-6 rounded-xl mb-8 text-center text-lg">
                    {firstName ? `Welcome, ${firstName}! ` : ''}Get personalized dietary and fitness plans tailored to your goals and preferences.
                    Our **professionally powered** system considers your unique profile to create the perfect plan for you.
                </div>
                ...
                <div className="mb-8">
                    ...
                    />
                    {!geminiApiKey && (
                        <p className="text-yellow-600 text-sm mb-4">⚠️ Please enter your Gemini API Key to proceed. <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline">Get your API key here</a></p>
                    )}
                    {geminiApiKey && (
                        <p className="text-green-600 text-sm mb-4">✅ API Key accepted!</p>
                    )}
                </div>
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            <label htmlFor="sex" className="block text-gray-700 text-sm font-medium mb-1">Sex</label>
                            <select
                                id="sex"
                                value={sex}
                        ...
                        <div>
                            <label htmlFor="activityLevel" className="block text-gray-700 text-sm font-medium mb-1">Activity Level</label>
                            <select
                                id="activityLevel"
                                value={activityLevel}
                        ...
                        <div>
                            <label htmlFor="dietaryPreferences" className="block text-gray-700 text-sm font-medium mb-1">Dietary Preferences</label>
                            <select
                        ...
                        <div>
                            <label htmlFor="fitnessGoals" className="block text-gray-700 text-sm font-medium mb-1">Fitness Goals</label>
                            <select
                                id="fitnessGoals"
                                value={fitnessGoals}
                        ...
                        <div>
                            <label htmlFor="mood" className="block text-gray-700 text-sm font-medium mb-1">Current Mood</label>
                            <select
                                id="mood"
                                value={mood}
                        ...
                        <div className="md:col-span-2">
                            <label htmlFor="stressLevel" className="block text-gray-700 text-sm font-medium mb-1">Current Stress Level</label>
                            <select
                                id="stressLevel"
                                value={stressLevel}
                ...
                {plansGenerated && (
                    <>
                        ...
                        <DisplayPsychologicalInsights insightsContent={psychologicalInsights} />

                        {/* Q&A Section */}
                        <div className="bg-white p-6 rounded-xl shadow-lg mb-8">
                            <h2 className="text-2xl font-bold text-gray-800 mb-4">❓ Questions about your plan?</h2>
                            <input
                                type="text"
                        ...
                        {qaPairs.length > 0 && (
                            <div className="bg-white p-6 rounded-xl shadow-lg">
                                <h2 className="text-2xl font-bold text-gray-800 mb-4">💬 Q&A History</h2>
                                <div className="space-y-6">
                                    {qaPairs.map((item, index) => (
                                        <div key={index} className="border-b border-gray-200 pb-4 last:border-b-0 last:pb-0">
                                            <p className="font-semibold text-indigo-700 mb-1">Q: {item.question}</p>
